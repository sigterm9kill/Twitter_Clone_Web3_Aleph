...
async function import_account({
  ...
  if (mnemonics) {
    let v = await mnemonicToSeed(mnemonics);
    let b = fromSeed(v);
...
async function pkey_sign(secretKey, address, message) {
  let buffer = get_verification_buffer$3(message);
  const signature = nacl$1.sign.detached(buffer, base58.decode(secretKey));
...
async function decrypt$1(account, content, { as_hex = true, as_string = true } = {}) {
  ...
  let result = null;
  if (curve == 'secp256k1') {
    result = decrypt$2(utils.decodeHex(account['private_key']), content);
  } else if (curve == 'secp256r1') {
    let opts = _decapsulate(content);
    result = await decrypt(utils.decodeHex(account['private_key']), opts);
...
async function encrypt$1(
  ...
  let result = null;
  if (curve == 'secp256k1')
    result = encrypt$2(target_publickey, encrypted_content);
  else if (curve == 'secp256r1') {
    result = await encrypt(utils.decodeHex(target_publickey), encrypted_content);
...
export { aggregates, avalanche, broadcast, cosmos, encryption, ethereum, ipfs_push, ipfs_push_file, messages, nuls, nuls2, posts, solana, storage_push, storage_push_file, store, substrate };
